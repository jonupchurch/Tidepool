...
Amendment (2.1.0): tuned for the Tidepools greenfield project — strengthened
...
discovered at the end. On this project specifically: default strongly to
asking — a batch of targeted clarifying questions up front is preferred over
proceeding on any unstated assumption.
...
(Principle IX) rather than replacing it. Keep the delivery brief and
skimmable — a TL;DR by default (lead with the answer, bullets over prose);
...
### X. Maintain Living Project Artifacts

Three root-level documents are kept current as the project moves — written
continuously, not once and abandoned:

- **`PLAN.md`** — the top-level project plan: vision, locked decisions, the
  whole feature set, and build order. The roadmap that frames the per-feature
  Spec-Kit specs/plans under `specs/`. Update it when scope, sequencing, or a
