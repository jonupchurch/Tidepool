...
- **Stack packs** (`stacks/`): repo-local conventions for this project's stack
  (a `stacks/tidepools.md` for the Vite + TS + Canvas + Tauri stack is
  forthcoming). Read the relevant one before writing code — these travel with
  the repo, unlike session-level plugin skills.
